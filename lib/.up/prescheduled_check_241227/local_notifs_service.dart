...
class LocalNotifsService {
  ...
  Future<void> scheduleTaskReminder(
    ...
    int minsSinceEpoch = dateTime.millisecondsSinceEpoch ~/ (1000 * 60);
    int id = int.parse('$taskId$minsSinceEpoch');

    final List<PendingNotificationRequest> pendingNotifs =
        await _flutterLocalNotificationsPlugin.pendingNotificationRequests();
    bool isPreScheduled = pendingNotifs.any((notif) => id == notif.id);
    if (!isPreScheduled) {
      // Schedule notifications
      // Join the two if conditions with AND
    }

    if (dateTime.isAfter(DateTime.now())) {
      await _flutterLocalNotificationsPlugin.zonedSchedule(
        id, title, body, tzDateTime, _notificationDetails,
      ...
      if (repeatOption == Repeat.daily) {
        await _flutterLocalNotificationsPlugin.periodicallyShow(
          id, title, body, RepeatInterval.daily, // Repeat daily [1]
          _notificationDetails,
          androidScheduleMode: AndroidScheduleMode.exactAllowWhileIdle,
        );
      }

      debugPrint('Done scheduling a reminder with id = $id at $tzDateTime');
      final pendingNotifs =
          await _flutterLocalNotificationsPlugin.pendingNotificationRequests();
