...
// [Test notifications (IDs, repeat modes); avoid duplicate rescheduling]
// [Add button doesn't pop now (check "if (task != null)")]
// [When I switch repeat to off, the repeat is still active I think because nothing tells local notifications that to cancel repeat]
// [If each task can have only one reminder, then adding datetime to notification id would create a new id for the same task each time we change date or time, thus having unwanted multiple reminders (old and new)]
// [Pass the whole class instead inside notification scheduling method, & test]
